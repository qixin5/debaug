...
const std::string usage_simple("Usage: gad [OPTIONS]... ORACLE PROGRAM");
const std::string error_message("Try 'gad --help' for more information.");
...
void OptionManager::showUsage() {
  llvm::errs()
      ...
      << "  --build                Integrate Gad with build system\n"
...
static const char *optstring = "ho:t:sDdglcLGCpvSm:i:a:e:k:j:";
...
std::string OptionManager::OutputDir = "debdce-out";
...
void OptionManager::handleOptions(int argc, char *argv[]) {
  ...
  if (optind + 2 > argc && !OptionManager::Stat) {
    llvm::errs() << "debdce: You must specify the oracle and input.\n";
    llvm::errs() << usage_simple << "\n";
    llvm::errs() << error_message << "\n";
    exit(1);
  } else if (optind + 1 > argc && OptionManager::Stat) {
    llvm::errs() << "debdce: You must specify the input.\n";
    exit(1);
  }

  if (optind + 1 > argc && OptionManager::Stat) {
    llvm::errs() << "debdce: You must specify the input.\n";
    exit(1);
  }
  ...
  if (Build) {
    // integrate Gad with build system
  ...
  // When Gad only computes statistics, it does not need to check the oracle
