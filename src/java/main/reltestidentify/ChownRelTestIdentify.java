...
public class ChownRelTestIdentify
{
	...
	for (String target_fline : target_flines) {
		...
		for (String elem : core_part.split("\\s+")) {
			...
			if (elem.startsWith("--reference")) {
			    option_flags.add("--reference");
			}
			else if (elem.startsWith("--from")) {
			    option_flags.add("--from");
			}
			else if (elem.startsWith("--version")) {
			    option_flags.add("--version");
			}
		    }
