...
public class Grep219RelTestIdentify
{
	...
	for (String target_fline : target_flines) {
		...
		for (String elem : core_part.split("\\s+")) {
			...
			if (elem.startsWith("--color")) {
			    option_flags.add("--color");
			}
			else if (elem.startsWith("--exclude-dir")) {
			    option_flags.add("--exclude-dir");
			}
			else if (elem.startsWith("--include")) {
			    option_flags.add("--include");
			}
		    }
